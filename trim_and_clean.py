...
"""
This script trim the adapters and remove reads mapping rRNAs.
...
def main():
    ...
    if rrna_input is not None:
        rrna = rrna_input
        if not os.path.exists(rrna):
            sys.exit("The rrna option is set, but the input does not exist")
    if output is None:
        output = os.getcwd()
    ...
    for sample_record in readrecord.samples:
        ...
        file_shell.close()
        # STAR
        #mis_cut = int(readrecord.sample2len[sample_name] * 0.9)
        mis_cut = 999  # large number to switch off
        ...
        if '2.4.' in star:
            write_shell_fastq_old(file_shell_str, paired_boolean, prefix, singleton)
        elif '2.5.' in star:
            write_shell_fastq(file_shell_str, paired_boolean, prefix,singleton)
        else :
            write_shell_fastq(file_shell_str, paired_boolean, prefix, singleton)
            print("Assuming the version of star is 2.5 and later \n")
...
def write_shell_trimmo(file_shell_str, loadtrimmo, trimmomatic, prefix, all_fastq_file,readrecord,sample_name,adaptor):
    ...
    file_shell.write("java -jar " + trimmomatic + ' ' + pe_tag + " -threads " + str(thread) + " " +
                     phred_tag + " \\\n" +
                     " " + all_fastq_file + " \\\n" +
                     " " + trim_out + " \\\n" +
...
def write_shell_star(file_shell_str, star, star_input, star_out_prefix, thread, rrna, mis_cut):
    with open(file_shell_str, "a") as shell_out:
        shell_out.write(star + ' --runThreadN ' + str(thread) + ' --genomeDir ' + rrna + " \\\n" +
                        ...
                        "  --outFilterMismatchNmax " + str(mis_cut) + " \\\n" +
...
def write_shell_fastq_old(file_shell_str, paired_boolean, prefix, singleton):
    ...
    if not paired_boolean:
        ...
        file_shell.write(''' sed '1~4 s|\s.\+$||g' < ''' + star_prefix + "Unmapped.out.mate1" +
                         ''' | paste - - - - | sort -k1,1 -S 10G -T ./ -V | tr '\\t' '\\n' >''' + final_out1 + "\n")
        file_shell.write(''' sed '1~4 s|\s.\+$||g' < ''' + star_prefix + "Unmapped.out.mate2" +
                         ''' | paste - - - - | sort -k1,1 -S 10G -T ./ -V | tr '\\t' '\\n' >''' + final_out2 + "\n")
        '''
        zcat reads.fq.gz \
        | paste - - - - \
...
def write_shell_fastq(file_shell_str, paired_boolean, prefix, singleton):
    ...
    if not paired_boolean:
        ...
        #file_shell.write("mv " + star_prefix + "Unmapped.out.mate1 " + final_out1 + "\n")
        #file_shell.write("mv " + star_prefix + "Unmapped.out.mate2 " + final_out2 + "\n")
        file_shell.write(''' sed '1~4 s|\s.\+$|/1|g' < ''' + star_prefix + "Unmapped.out.mate1" +
                         ''' | paste - - - - | sort -k1,1 -S 10G -T ./ -V | tr '\\t' '\\n' >''' + final_out1 + "\n")
        file_shell.write(''' sed '1~4 s|\s.\+$|/2|g' < ''' + star_prefix + "Unmapped.out.mate2" +
                         ''' | paste - - - - | sort -k1,1 -S 10G -T ./ -V | tr '\\t' '\\n' >''' + final_out2 + "\n")

        #####################################
        read1_ori = prefix + '_STAR_S1_' + 'Unmapped.out.mate1'
        read2_ori = prefix + '_STAR_S2_' + 'Unmapped.out.mate1'
        ## convert into old version
        file_shell.write(''' sed -i '1~4 s|$|/1|g' ''' + read1_ori  + "\n")
        file_shell.write(''' sed -i '1~4 s|$|/1|g' ''' + read2_ori + "\n")
